<script lang="ts">
	import '$lib/scss/components/public/announcement.scss';
	...
	{#each announcements as announcement (announcement.id)}
		<div class="announcement">
			...
			<div class="info">
				...
				{/if}
				<p class="date">{new Date(announcement.createdAt).toLocaleDateString()}</p>
				{#if announcement.fixed}
					<p class="fixed"><Icon src={BsPinAngle} /></p>
				{/if}
			</div>
		</div>
	{/each}
